...
export function CycleHistoryRow({ cycle, index }: { cycle: ReturnType<typeof selectCycleHistory>[0]; index: number }) {
  ...
  return (
    <Animated.View entering={FadeInDown.delay(index * 50).springify()} style={hist_s.row}>
      <View style={[hist_s.indicator, { backgroundColor: hasSurplus ? '#68D391' : '#FC8181' }]} />
      <View style={{ flex: 1 }}>
        <Text style={hist_s.range}>
          {format(new Date(cycle.startDate), 'dd MMM', { locale: es })} →{' '}
          {format(new Date(cycle.endDate), 'dd MMM yyyy', { locale: es })}
        </Text>
        <Text style={hist_s.detail}>
          Gastado ${cycle.totalSpent.toLocaleString()} de ${cycle.effectiveBudget.toLocaleString()}
          {cycle.rolloverBonus > 0 ? ` (+$${cycle.rolloverBonus} rollover)` : ''}
        </Text>
      </View>
      <Text style={[hist_s.amount, { color: hasSurplus ? '#68D391' : '#FC8181' }]}>
